...
NRES_D   = 2;                  % 
NSPC     = [50 50];          % Buffer of free space before and after device
...
for nd = 1 : length(DDAT)
    ...
    x1     = nz1*dz/millimeters
    % Fill xx array with stop and start values for visualisation of layers
    xx(nd, 1) = x1;  % These lines draw closed rectangle with yy (fixed)
    xx(nd, 4) = x1;
    xx(nd, 5) = x1;      
    % update next nz1 start index for next layer
    nz1    = nz2 + 1;
    
    x2     = nz1*dz/millimeters
    xx(nd, 2) = x2;
    xx(nd, 3) = x2;
end
%xx    % Uncomment to check xx array for visualisation of layers

% CALCULATE SLABS TO PLOT FOR VISUALISATION !!! ADD TO LOOP
yy = [ -2 -2 +2 +2 -2 ];  % Disitance of y dimension should be heigher than plot 
...
for n = 1 : N
  ...
  if ~mod(n,50)
    ...
    for nd = 1 : length(DDAT)
    if ~mod(nd,2)
       fill(xx(nd,[1 2 3 4 5]),yy,'r','FaceAlpha',0.2);      %% loop for slabs
    else
       fill(xx(nd,[1 2 3 4 5]),yy,'b','FaceAlpha',0.2);
    end
    hold on;
    end   
    % Plot Fields
    ...
    ylim([-80 5]); % Change limits to suit...
    xlabel('Frequency (GHz)');
    ylabel('Reflection & Transmission (dB)');
